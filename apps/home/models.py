...
class EducationActivity(models.Model):
    ...
    education = models.ForeignKey(
        "home.Education", on_delete=models.PROTECT, related_name="education_activities"
    )
...
class ExperienceActivity(models.Model):
    ...
    programming_tools = models.ManyToManyField(
        "programming.ProgrammingTool", related_name="experience_activities", blank=True
    )
    url = models.URLField(blank=True, default="")
    is_featured = models.BooleanField(default=False)
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def __str__(self):
        description = (
            self.description[:50]
            if len(self.description) > 49
            else self.description[:len(self.description)]
        )
        return f"{self.experience}: {description}"


class Experience(models.Model):
    position = models.CharField(max_length=80)
    employer = models.ForeignKey(
        Employer, on_delete=models.PROTECT, related_name="experiences"
    )
...
class Education(models.Model):
    ...
    institute = models.ForeignKey(
        EducationalInstitute, on_delete=models.PROTECT, related_name="institutes"
    )
    short_summary = models.TextField()
    started_at = models.DateField()
    ended_at = models.DateField(null=True, blank=True)
...
class Publication(models.Model):
    ...
    def __str__(self):
        return (
            ...
            else self.title[: len(self.title)]
        )


class PersonalSkill(models.Model):
    category = models.ForeignKey(
        "programming.Category", on_delete=models.PROTECT, related_name="personal_skills"
    )
    title = models.CharField(max_length=300, blank=True, default="")
    programming_tool = models.ForeignKey(
        "programming.ProgrammingTool",
        related_name="presentation_skills",
        null=True,
        blank=True,
